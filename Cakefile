...
task 'coffeeFiles', 'how much coffee you got?!', ->
  ...
  ##we add the exportFile at the end
  appFiles.push 'export.coffee'
  util.log 'export.coffee'
...
task 'build', 'Build single application file from source files', ->
  ...
  process = ->
    fs.writeFile 'bin/timer.coffee', appContents.join('\n\n'), 'utf8', (err) ->
      throw err if err
      exec 'coffee --compile bin/timer.coffee', (err, stdout, stderr) ->
        if err
          util.log 'Error compiling coffee file.'
          grrrr 'Uh, your coffee is bad.'
        else
          fs.unlink 'bin/timer.coffee', (err) ->
            if err
